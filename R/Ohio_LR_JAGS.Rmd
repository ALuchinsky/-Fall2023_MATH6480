...
```{r}
j=8
thM %>% ggplot(aes(x=th.8.)) + geom_histogram(bins = 100) + geom_vline(xintercept = ohio.data$th[8], col="red", lty=2) +
  xlim(0.37, 0.38) + xlab("theta") + ggtitle(paste0(ohio.data$county.name[8], " County"))
save_fig("byCounty/Ohio_LR_comparison_Brown")
...
```{r}
i=8
hist(M[,4+i], main=colnames(M)[4+i], xlim=c(0.37,0.38))
abline(v=ohio.data$th[i])
```
