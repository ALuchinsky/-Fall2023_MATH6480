...
```{r}
df <- all_data %>% 
  dplyr::select(year, state, party, candidatevotes) %>% 
...
df %>% filter(year == 2000) %>% tidyr::pivot_longer(cols = c(D, R)) %>% 
  ggplot(aes(x=state, y=value, fill=name)) + geom_col(position = "fill") +
...
```{r}
dfM %>% ggplot(aes(x=a, y=b)) + geom_point(cex=0.1) +
  ggtitle("Year = 2004")
...
```{r}
dfM %>% ggplot(aes(x=mu, y=piTau)) + geom_point(cex = 0.5)
...
```{r}
pDIC <- apply(M[,-c(1,2)], 1, compLogP,
  y=filter(scaled_data, year == 2000)
)
```

```{r}
DIC.binom  <- 2*(compLogP(theta = thBayes, y=filter(scaled_data, year == 2000)) - mean(pDIC))
DIC.binom
```
